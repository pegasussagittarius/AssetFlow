...
import { Wallet, LogOut, Plus, X, Sparkles, Download, Upload, Settings, FileText, Calculator, CreditCard, User as UserIcon, Target } from 'lucide-react';
...
import { Asset, User, AssetCategoryType, RiskLevel, AssetTermType, GoogleSyncConfig } from './types';
import { ASSET_CATEGORIES, ASSET_TERMS, DEFAULT_GOLD_PRICE, DEFAULT_EXCHANGE_RATE, RISK_PROFILES } from './constants';
import { initGoogleClient, backupToDrive, restoreFromDrive } from './services/googleDriveService';
...
export default function App() {
  ...
  // --- Google Sync Logic ---
  const handleGoogleSync = async (action: 'backup' | 'restore', config: GoogleSyncConfig) => {
    if (!currentUser) return;
    
    // 1. Initialize Google Client
    await initGoogleClient(config.apiKey, config.clientId);
    ...
    if (action === 'backup') {
        ...
        if (data && data.assets && Array.isArray(data.assets)) {
             ...
             // Optionally restore user settings if structure matches
             if (data.user) {
                 updateUserStorage({...currentUser, ...data.user});
             }
        } else {
            throw new Error("Dữ liệu từ Drive không đúng định dạng.");
        }
    }
  };
  ...
  return (
    <div className={`min-h-screen transition-colors duration-200 p-4 md:p-8 font-sans ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      <header className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
             {currentUser.avatar ? (
                <img src={currentUser.avatar} alt="Avatar" className="w-12 h-12 rounded-full object-cover border-2 border-blue-500 shadow-sm" />
             ) : (
            ...
            <div>
              ...
              <div className="flex flex-col gap-1 mt-1">
                {currentUser.creditScore && (
                    ...
                    className={`inline-flex items-center gap-2 px-2 py-1 rounded text-xs font-medium border shadow-sm transition-all hover:scale-105 cursor-pointer w-fit ${
                    isDarkMode ? 'bg-slate-800 border-slate-700 text-emerald-400 hover:bg-slate-700' : 'bg-white border-blue-200 text-blue-700 hover:bg-blue-50'}`}>
                ...
                {currentRiskProfile ? (
                    <button 
                        onClick={() => setIsRiskSurveyOpen(true)}
                        title="Xem/Điều chỉnh hồ sơ rủi ro"
                        className={`inline-flex items-center gap-2 px-2 py-1 rounded text-xs font-medium border shadow-sm transition-all hover:scale-105 cursor-pointer w-fit ${
                            isDarkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-blue-200 hover:bg-blue-50'
                        }`}
                        style={{ color: currentRiskProfile.color }}
                    >
                        <Target className="w-3 h-3" />
                        <span>Hồ sơ: <span className="font-bold">{currentRiskProfile.name}</span></span>
                    </button>
                ) : (
                     <button 
                        onClick={() => setIsRiskSurveyOpen(true)}
                        title="Làm khảo sát để xác định khẩu vị rủi ro"
                        className={`inline-flex items-center gap-2 px-2 py-1 rounded text-xs font-medium border border-dashed transition-all hover:scale-105 cursor-pointer w-fit ${
                            isDarkMode ? 'border-slate-600 text-slate-400 hover:text-white hover:border-slate-500' : 'border-gray-300 text-gray-500 hover:text-blue-600 hover:border-blue-300'
                        }`}
                    >
                        <Target className="w-3 h-3" />
                        <span>Chưa xác định khẩu vị đầu tư</span>
                    </button>
                )}
              </div>
            </div>
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <button onClick={() => setIsExportModalOpen(true)} title="Xuất báo cáo PDF" className={`p-2.5 rounded-lg transition-colors border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-rose-400 hover:bg-slate-700' : 'bg-white border-gray-200 text-rose-600 hover:bg-gray-50'}`}>
            <FileText className="w-5 h-5" />
          </button>

          <button onClick={handleExportData} title="Xuất dữ liệu (Backup JSON)" className={`p-2.5 rounded-lg transition-colors border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-emerald-400 hover:bg-slate-700' : 'bg-white border-gray-200 text-emerald-600 hover:bg-gray-50'}`}>
            <Download className="w-5 h-5" />
          </button>
          
          <button onClick={handleImportClick} title="Nhập dữ liệu (Restore)" className={`p-2.5 rounded-lg transition-colors border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-orange-400 hover:bg-slate-700' : 'bg-white border-gray-200 text-orange-600 hover:bg-gray-50'}`}>
            <Upload className="w-5 h-5" />
          </button>

          <div className={`h-8 w-[1px] mx-1 ${isDarkMode ? 'bg-slate-700' : 'bg-slate-200'}`}></div>

          <button onClick={() => setIsSettingsModalOpen(true)} title="Cài đặt" className={`p-2.5 rounded-lg transition-colors border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border-gray-200 text-slate-600 hover:bg-gray-50'}`}>
      ...
      <AIAdvisorModal isOpen={isAIModalOpen} onClose={() => setIsAIModalOpen(false)} assets={currentAssets} totalValue={totalValue} isDark={isDarkMode} currentUser={currentUser} />
      ...
      <RiskSurveyModal isOpen={isRiskSurveyOpen} onClose={() => setIsRiskSurveyOpen(false)} isDark={isDarkMode} onSaveProfile={handleSaveRiskProfile} currentProfileId={currentUser.riskProfile} />
      
      <CICInfoModal isOpen={isCICInfoOpen} onClose={() => setIsCICInfoOpen(false)} isDark={isDarkMode} currentScore={currentUser.creditScore} />

      <ExportReportModal isOpen={isExportModalOpen} onClose={() => setIsExportModalOpen(false)} assets={assets} currentUser={currentUser} isDark={isDarkMode} />
                ...
                <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-yellow-900/10 border-yellow-500/30' : 'bg-yellow-50 border-yellow-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                     <div>
                       ...
                       <input type="number" placeholder="0" min="0" step="0.01" className={`w-full p-2 rounded border outline-none ${isDarkMode ? 'bg-slate-800 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={goldInputs.taels} onChange={e => setGoldInputs({...goldInputs, taels: e.target.value})} />
                     </div>
                     <div>
                       <label className={`block text-xs mb-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Số Lượng (Chỉ)</label>
                       <input type="number" placeholder="0" min="0" step="0.1" className={`w-full p-2 rounded border outline-none ${isDarkMode ? 'bg-slate-800 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={goldInputs.mace} onChange={e => setGoldInputs({...goldInputs, mace: e.target.value})} />
                     </div>
                  </div>
                  <div>
                    <label className={`block text-xs mb-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Đơn giá 1 Chỉ (VND)</label>
                    <input type="number" className={`w-full p-2 rounded border outline-none ${isDarkMode ? 'bg-slate-800 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={goldInputs.pricePerMace} onChange={e => setGoldInputs({...goldInputs, pricePerMace: e.target.value})} />
                  </div>
                </div>
              )}
                ...
                <div className={`p-4 rounded-xl border ${isDarkMode ? 'bg-teal-900/10 border-teal-500/30' : 'bg-teal-50 border-teal-200'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                     <div>
                       ...
                       <input type="number" placeholder="0" min="0" step="0.01" className={`w-full p-2 rounded border outline-none ${isDarkMode ? 'bg-slate-800 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={fcInputs.quantity} onChange={e => setFcInputs({...fcInputs, quantity: e.target.value})} />
                     </div>
                     <div>
                       <label className={`block text-xs mb-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Tỷ giá (VND)</label>
                       <input type="number" placeholder="0" className={`w-full p-2 rounded border outline-none ${isDarkMode ? 'bg-slate-800 border-slate-600 text-white' : 'bg-white border-gray-300'}`} value={fcInputs.exchangeRate} onChange={e => setFcInputs({...fcInputs, exchangeRate: e.target.value})} />
                     </div>
                  </div>
                </div>
              )}
